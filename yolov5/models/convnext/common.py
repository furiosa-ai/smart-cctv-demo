...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        if global_config["force_act"] is not None:
            act_func = eval(global_config["force_act"])
        else:
            act_func = nn.SiLU()
...
class CoordinateLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        n, c, h, w = x.size()
        x_h = self.pool_h(x)  # [n,c,h,w]-->[n,c,h,1]
        x_w = self.pool_w(x).permute(0, 1, 3, 2)  # [n,c,h,w]-->[n,c,1,w]-->[n,c,w,1]

        y = torch.cat([x_h, x_w], dim=2)  # -->[n,c,(h+w),1]
        y = self.conv1(y)  # [n,c,(h+w),1]-->[n,mip,(h+w),1]
        y = self.bn1(y)  # [n,mip,(h+w),1]
        y = self.act(y)  # [n,mip,(h+w),1]

        x_h, x_w = torch.split(y, [h, w], dim=2)  # [n,mip,(h+w),1]-->[n,mip,h,1] & [n,mip,w,1]
        x_w = x_w.permute(0, 1, 3, 2)  # [n,mip,w,1]-->[n,mip,1,w]

        a_h = self.conv_h(x_h).sigmoid()  # [n,mip,h,1]-->[n,oup,h,1]-->x轴概率分布
        a_w = self.conv_w(x_w).sigmoid()  # [n,mip,w,1]-->[n,oup,w,1]-->y轴概率分布
...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        self.norm = LayerNorm(dim, eps=1e-6) if not global_config["no_ln"] else BatchNormModule(dim, eps=1e-6)
...
class LayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-6, data_format="channels_last"):
        ...
        assert not global_config["no_ln"]
...
class ConvNeXt_Block(nn.Module):  # index 0~3
    def __init__(self, index, in_chans, depths, dims, drop_path_rate=0., layer_scale_init_value=1e-6):
        ...
        stem = nn.Sequential(
            ...
            LayerNorm(dims[0], eps=1e-6, data_format="channels_first") if not global_config["no_ln"] else BatchNormModule(dims[0], eps=1e-6, data_format="channels_first")
        )
        self.downsample_layers.append(stem)
        for i in range(3):
            downsample_layer = nn.Sequential(
                LayerNorm(dims[i], eps=1e-6, data_format="channels_first") if not global_config["no_ln"] else BatchNormModule(dims[i], eps=1e-6, data_format="channels_first"),

...
def txt2img_iter(show_video_name, dataset, track_results=None, out_path="out/videos/"):
    ...
    gt_json_path = f'../data/{dataset}/annotations/val_half.json'
    img_path = f'../data/{dataset}/train/'
    ...
    vis_gt = track_results is None

    img_dict = dict()
    
    if vis_gt:
        txt_path = f'../data/{dataset}/train/{show_video_name}/gt/gt_val_half.txt'
    else:
        txt_path = os.path.join(track_results, 'track_results', show_video_name + '.txt')
    ...
    if visual_path == "visual_val_gt":
        ...
        txt_path = os.path.join(res_path, 'track_results', show_video_name + '.txt')
    ...
    def _iter():
        for img_id in sorted(txt_dict.keys()):
            ...
            # cv2.imwrite(os.path.join(out_path, show_video_name + "{:0>6d}.png".format(img_id)), img)
            yield img

    return iter(_iter()), len(txt_dict)
        
def txt2vid(show_video_name, dataset, track_results=None, out_path="out/videos/"):
    vid_path = os.path.join(out_path, f"{show_video_name}.mp4")

    fps = 16 
    size = (1920,1080) 
    videowriter = cv2.VideoWriter(vid_path,cv2.VideoWriter_fourcc('M','J','P','G'), fps, size)

    imgs, num_imgs = txt2img_iter(show_video_name, dataset, track_results, out_path)

    for img in tqdm(imgs, total=num_imgs):
...
def main():
    ...
    # txt2img(["MOT20-01"], "MOT20", track_results="out/res_save/bytetrack_mot20_5data", out_path="out/videos/")
    txt2vid("MOT20-01", "MOT20", track_results="out/res_save/bytetrack_mot20_5data", out_path="out/videos/")

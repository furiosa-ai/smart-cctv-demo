...
def main(args):
    ...
    backbone = get_model(
        cfg.network, dropout=0.0, fp16=cfg.fp16, num_features=cfg.embedding_size, act=cfg.act).cuda()
    ...
    if cfg.weights is not None:
        ...
        if cfg.act == "leakyrelu":
            checkpoint = OrderedDict((k, v) for k, v in checkpoint.items() if ("prelu" not in k))

        backbone.module.load_state_dict(checkpoint)
        print(f"Loaded checkpoint {cfg.weights}")
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        if rank == 0:
            path_module = os.path.join(cfg.output, f"model_epoch{epoch:04d}.pt")

...
from utils.yolo_qat import Yolov5QATModel
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    last = wdir / 'last_quant.pt'
    best = wdir / 'best_quant.pt'
    ...
    # model_call(model, "set_act", nn.ReLU)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            quantized_model = torch.quantization.convert(model.cpu().eval(), inplace=False).eval()
            ...
            if save:
                ...
                """
                torch.save(quantized_model.state_dict(), last)
                if best_fitness == fi:
                    torch.save(quantized_model.state_dict(), best)

...
def model_call(model, func_name, *args, force=False):
    for name, module in model._modules.items():
        ...
        model_call(module, func_name, *args)
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=()):  # detection layer
        ...
        self.export_cat = False  # onnx export cat output
        self.export_warboy = False

    def prepare_qat(self):
        self.dequant = torch.quantization.DeQuantStub()

    # @torch.jit.unused
    def _forward_export_cat(self, x):
        z: List[torch.Tensor] = []
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    act = d.get("config", {}).get("act", None)
    if act is not None:
        model_call(m, "set_act", eval(act))

    return m, sorted(save)

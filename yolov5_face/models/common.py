...
from utils.qat import QATAdd, QATConcat
...
class Conv(nn.Module):
    ...
    def prepare_qat(self):
        assert isinstance(self.act, nn.ReLU)
...
class StemBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.stem_3(self.cat((stem_2b_out,stem_2p_out),1))
        return out
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        ...
        self.add = QATAdd() if shortcut and c1 == c2 else None

    def prepare_qat(self):
        pass

    def forward(self, x):
        return self.add(x, self.cv2(self.cv1(x))) if self.add is not None else self.cv2(self.cv1(x))
